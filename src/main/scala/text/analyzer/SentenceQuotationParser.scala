...
object SentenceQuotationParser {
  ...
  private final val quotations: Seq[Quotation] = {
    val buffer: ListBuffer[Quotation] = ListBuffer.empty[Quotation]
    Source.fromFile(
  ...
  def getFirstMatchOpt(sentenceOpt: StringOption): Option[(Quotation, Range)] = {
    ...
    val quotationRangeBuffer: ListBuffer[(Quotation, Range)] = ListBuffer.empty[(Quotation, Range)]
  ...
  def splitAndQuotationParseJapaneseText(textOpt: StringOption): Seq[NormalizedQuotedSentence] = {
    val buffer: ListBuffer[NormalizedQuotedSentence] = ListBuffer.empty[NormalizedQuotedSentence]
